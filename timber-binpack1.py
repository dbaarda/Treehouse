...
glam = {2700:8, 870:3, 330:1}
pine = {4800:5, 3000:2, 2860:1}

# At 2024-03-027
glam = {2700:4, 870:1, 720:1, 710:1, 570:1, 330:1}
pine = {4800:9, 3000:2, 2860:1, 2830:1, 2117:1, 660:1}
...
rearwall = (
    ...
    + 3 * [90]
    + 4 * [65])
rearwall.sort(reverse=True)

leftwall_g = sorted([2123, 1985, 1972, 1834, 90], reverse=True)
leftwall_p = sorted(
     + 1 * [2875, 2860]
     + 1 * [2076, 2030, 1926, 1880]
     + 2 * [415]
     + 4 * [412.5], reverse=True)
leftwall = sorted(leftwall_g + leftwall_p, reverse=True)

rightwall_g = sorted([2123, 1985, 1972, 1834], reverse=True)
rightwall_p = sorted(4*[1260])
rightwall_o = sorted(
    + 1 * [2875, 2860]
    + 1 * [2117, 1839]
    + 2 * [1260]
    + 4 * [1000]
    + 1 * [308, 264, 162, 118, 90, 70], reverse=True)
rightwall = sorted(rightwall_g + rightwall_p + rightwall_o, reverse=True)

frontwall_p = sorted(
  + 2 * [760]
  + 2 * [660]
  + [650, 580], reverse=True)
frontwall_o = sorted(
    + 2 * [3000]
    + 9 * [2130]
    + 1 * [660]
    + [650]
    + [365,230,90]
    + [1000,700,520,365,70], reverse=True)
frontwall = sorted(frontwall_p + frontwall_o, reverse=True)

allwall = sorted(rearwall+rightwall+leftwall+frontwall,reverse=True)
partwall = sorted(rightwall+leftwall+frontwall, reverse=True)
sidewall= sorted(leftwall+rightwall ,reverse=True)

store = Lengths(supply)
gstock = Lengths(glam)
pstock = Lengths(pine)
astock = gstock+pstock
print(f'total available glam stock: {gstock}\n')
print(f'total available pine stock: {pstock}\n')
print(f'total available stock: {astock}\n')
...
print('Do glam stock and parts first, then other parts.')
stock = Lengths(glam)
print(f'total glam stock: {stock}')
bms = Beams(stock)
parts = rightwall_g
print(f'glam parts needed: {fmtlens(parts)}')
for p in parts:
  bms.addcut(p)
parts = sorted(rightwall_o + frontwall_o, reverse=True)
print(f'other parts needed: {fmtlens(parts)}')
for p in parts:
  bms.addcut(p)
print(bms)
print()

# Get the other parts that didn't fit in glam stock.
oparts=bms.missing

print('Do pine stock and parts with other parts.')
stock = Lengths(pine)
print(f'total pine stock: {stock}')
bms = Beams(stock)
parts = sorted(rightwall_p + frontwall_p + oparts, reverse=True)
...
if bms.missing:
  print('Remaining parts and additional stock needed;')
  nbs = Beams(store)
  parts = sorted(bms.missing, reverse=True)
  print(f'parts needed: {fmtlens(parts)}')
  for p in parts:
    nbs.addcut(p)
  print(nbs)
  cost = sum(b.length*cost_mm for b in nbs)
  print(f'cost=${cost:.2f}')
